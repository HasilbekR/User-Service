...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<StandardResponse<String>>
        ...
        return ResponseEntity.status(400).body(StandardResponse.<String>builder().status("400").message(e.getMessage()).build());
    }
    ...
    public ResponseEntity<StandardResponse<String>> authenticationFailedExceptionHandler(
        ...
        return ResponseEntity.status(401).body(StandardResponse.<String>builder().status("401").message(e.getMessage()).build());
    ...
    public ResponseEntity<StandardResponse<String>> AccessDeniedExceptionHandler(
        ...
        return ResponseEntity.status(403).body(StandardResponse.<String>builder().status("403").message(e.getMessage()).build());
    }

    @ExceptionHandler(value = {DataNotFoundException.class})
    public ResponseEntity<StandardResponse<String>> dataNotFoundExceptionHandler(
            DataNotFoundException e){
        return ResponseEntity.status(404).body(StandardResponse.<String>builder().status("404").message(e.getMessage()).build());
