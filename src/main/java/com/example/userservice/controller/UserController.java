...
import com.example.userservice.domain.dto.request.doctor.*;
...
//    }
    ...
    @GetMapping("/countActiveDoctorBookingAndQueues")
    @PreAuthorize(value = "hasRole('DOCTOR') and hasRole('ADMIN')")
    public StandardResponse<Long> countActiveBookingAndQueues(
            @RequestParam UUID doctorId
    ) {
        return doctorService.countActiveDoctorBookingAndQueues(doctorId);
    }


    @GetMapping("/countCompleteDoctorBookingAndQueues")
    @PreAuthorize(value = "hasRole('DOCTOR') and hasRole('ADMIN')")
    public StandardResponse<Long> countCompleteDoctorBookingAndQueues(
            @RequestParam UUID doctorId
    ) {
        return doctorService.countCompleteDoctorBookingAndQueues(doctorId);
    }
